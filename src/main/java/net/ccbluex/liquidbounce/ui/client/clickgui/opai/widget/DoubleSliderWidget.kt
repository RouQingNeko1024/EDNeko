...
class DoubleSliderWidget(setting: DoubleSetting) : SettingWidget<DoubleSetting>(setting) {

    private val format = DecimalFormat("#0.00")
    private val valueFieldWidth = 40.0f
    private val valueFieldHeight = 12.0f

    private val inputField = OpaiTextField(16) { it.matches("[0-9.\\-]".toRegex()) }
    ...
    override fun draw(renderer: OpaiRenderer, mouseX: Int, mouseY: Int) {
        val ratio = ((setting.getDoubleValue() - setting.min) / (setting.max - setting.min)).toFloat()
        ...
        var valueStr = if (inputField.focused) inputField.text else formatPlainValue()
        if (!inputField.focused && setting.isPercentageMode()) valueStr += "%"
        ...
        if (dragging) {
            ...
            val value = setting.min + Math.round(rawRatio * range / step) * step
            setting.setDoubleValue(MathHelper.clamp_double(value, setting.min, setting.max))
        }
    }

    override fun mouseClicked(mouseX: Double, mouseY: Double, button: Int): Boolean {
        if (button == 0) {
            val plainValue = formatPlainValue()
    ...
    override fun keyPressed(keyCode: Int, scanCode: Int, modifiers: Int): Boolean {
        ...
        if (keyCode == Keyboard.KEY_ESCAPE) {
            inputField.text = formatPlainValue()
            inputField.blur()
            return true
        }
    ...
    private fun formatPlainValue(): String = format.format(setting.getDoubleValue())
