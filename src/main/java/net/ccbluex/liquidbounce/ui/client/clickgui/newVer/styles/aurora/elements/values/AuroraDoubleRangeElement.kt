...
class AuroraDoubleRangeElement(value: DoubleRangeValue) : AuroraValueElement<ClosedFloatingPointRange<Double>>(value) {
    ...
    override fun drawElement(mouseX: Int, mouseY: Int, x: Float, y: Float, width: Float, accentColor: Color): Float {
        val rangeValue = value as DoubleRangeValue
        valueHeight = 40F

        val range = rangeValue.get()
        val progressLeft = ((range.start - rangeValue.minimum) / (rangeValue.maximum - rangeValue.minimum)).toFloat()
        val progressRight = ((range.endInclusive - rangeValue.minimum) / (rangeValue.maximum - rangeValue.minimum)).toFloat()

        animLeft += (progressLeft - animLeft) * 0.15F
        animRight += (progressRight - animRight) * 0.15F

        val text = "${rangeValue.name}: ${formatValue(range.start)} - ${formatValue(range.endInclusive)}${rangeValue.suffix ?: ""}"
    ...
    override fun onClick(mouseX: Int, mouseY: Int, x: Float, y: Float, width: Float): Boolean {
        ...
        val range = rangeValue.get()
        val leftX = sliderX + sliderWidth * ((range.start - rangeValue.minimum) / (rangeValue.maximum - rangeValue.minimum)).toFloat()
        val rightX = sliderX + sliderWidth * ((range.endInclusive - rangeValue.minimum) / (rangeValue.maximum - rangeValue.minimum)).toFloat()
    ...
    private fun updateValue(mouseX: Int, x: Float, width: Float) {
        ...
        val sliderX = x
        val sliderWidth = width - 10F
        val newProgress = ((mouseX - sliderX) / sliderWidth).coerceIn(0F, 1F).toDouble()
    ...
    private fun formatValue(d: Double): String {
        if (d.isNaN() || d.isInfinite()) return "0.00"
        val absVal = abs(d)
        return when {
            d == d.toLong().toDouble() && absVal < 1e15 -> d.toLong().toString()
            absVal >= 1e7 || (absVal > 0.0 && absVal < 1e-3) -> String.format("%.2e", d)
